...
public class DiscreteEmpiricGen implements Gen<Integer> {
    private record InnerGen(int from, int to, Random gen) {
        public int next() {
            return gen.nextInt(from, to);
        }
    }

    public record InputWrapper(int from, int to, double probability) {
    }
    ...
    public DiscreteEmpiricGen(InputWrapper... rangeAndProbability) {
